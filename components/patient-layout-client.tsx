...
export function PatientLayoutClient({ children }: PatientLayoutClientProps) {
  ...
  // Handle authentication check - only redirect after loading is complete and we've checked auth
  useEffect(() => {
    if (!loading) {
      setHasCheckedAuth(true)

      // Only redirect if we're not on an auth page and there's no user
  ...
  // If no user and not on auth page, show redirecting message
  if (!user && !session && !pathname.startsWith("/portal/auth/")) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="text-center">
